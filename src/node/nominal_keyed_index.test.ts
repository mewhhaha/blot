...
test("nominal keyed indexes preserve key domains in both executions", async () => {
...
test("nominal keyed indexes reject a key from another keyspace", async () => {
  const compiler = await Compiler.create();
  try {
    await assert.rejects(() => compiler.check(mismatchFixture), /BLOT_TYPE_ERROR/);
  } finally {
    compiler.destroy();
  }
});

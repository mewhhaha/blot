...
import { prepareGpupaperHir } from "../conformance/gpufuck/compile.ts";
import { buildTypeScriptOracleBatch } from "../conformance/gpufuck/gpupaper.ts";
import {
  type BlotAbiManifest,
  buildBlotAbiManifest,
} from "../conformance/gpufuck/runtime/abi.ts";
import { decodeBlotAbiResult } from "../conformance/gpufuck/runtime/abi_decode.ts";
import {
...
import { prepareRuntimeHir } from "./session.ts";
...
Deno.test("Rust HIR matches the bounded TypeScript ABI oracle", async () => {
  const rust = validateBlotRuntimeModule(
    await prepareRuntimeHir("examples/minimal.blot"),
  );

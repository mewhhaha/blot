import type { BlotAbiFunction, BlotAbiManifest, BlotAbiType } from "./abi.ts";
...
export function compileBlotCanonicalTextAbi(
  ...
  for (const literal of literals) {
    let pointer = staticEnd;
    if (literal.contents.byteLength === 0) pointer = 0;
  ...
  for (const imported of manifest.imports) {
    ...
    let resultTypes: number[] = [];
    if (results.length <= 1) resultTypes = results.map(flatWasmType);
...
function checkedI64ScalarInstructions(
  ...
  let arithmetic = wasmInstruction.i64Multiply;
  if (operator === "add") arithmetic = wasmInstruction.i64Add;
  if (operator === "subtract") arithmetic = wasmInstruction.i64Subtract;
  const instructions: WasmInstruction[] = [
    ...wasmInstruction.localGet(left),
    ...wasmInstruction.localGet(right),
    ...arithmetic,

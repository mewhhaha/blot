...
import { buildBlotAbiManifest, requireDirectBlotAbiBoundary } from "./abi.ts";
import { decodeBlotAbiResult } from "./abi_decode.ts";
import {
...
Deno.test("Blot Runtime target validates and reclaims dynamic canonical Text", async () => {
  ...
  for (const expected of ["", "Ada", "Łucja 🦆"]) {
    ...
    call(instance, "blot:default");
    let greeting = `Hello, ${expected}`;
    if (expected === "") greeting = "Hello, stranger.";
    assertEquals(
      messages.join("|"),
...
function dynamicTextModule(
  ...
  let comparisonText = options.comparisonText;
  if (comparisonText === undefined) comparisonText = "";
  let comparisonOperator = options.comparisonOperator;
    ...
    functions: [{
      ...
      blocks: [{
        ...
        operations: [
          ...
          textOperation(4, comparisonText),
          {
            kind: "text.compare",
            result: 5,
            type: 2,
...
async function instantiate(
  ...
  const heap = createRuntimeHeap(textLiterals);
  const compiled = await WebAssembly.compile(
    new Uint8Array(wasm).buffer as ArrayBuffer,
  );
  return await WebAssembly.instantiate(compiled, {
    [coreRuntimeImportModule]: createRuntimeImports(heap),
  });
}

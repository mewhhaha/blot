import type { BlotAbiManifest, BlotAbiType } from "./abi.ts";
import { flattenedAbiType } from "./abi.ts";
import type { BlotRuntimeFunction, BlotRuntimeModule } from "./hir.ts";
...
export function compileBlotConstantAbi(
  ...
  const templates = planned.map((exported) => {
    ...
    if (flattenedAbiType(type).length > 1) {
      return new CanonicalTemplate(type, exported.evaluation.value).finish();
    }
    return null;
  });
  let staticEnd = 1_024;
...
function directConstant(
  ...
  if (type.kind === "boolean" && typeof value === "boolean") {
    let encoded = 0;
    if (value) encoded = 1;
    return wasmInstruction.i32Constant(encoded);
  }
  ...
  if (type.kind === "record" && isProduct(value)) {
    const flattened = type.fields.flatMap((field) => {
      if (flattenedAbiType(field.type).length === 0) return [];
      return [{ type: field.type, value: value.fields.get(field.name)! }];
    });
...
function flatWasmType(type: "i32" | "i64" | "f32" | "f64"): number {
  if (type === "i32") return wasmType.i32;
  if (type === "i64") return wasmType.i64;
  if (type === "f32") return wasmType.f32;
  return wasmType.f64;
}
...
function constantMismatch(type: BlotAbiType, value: ConstantValue): TypeError {
  let valueKind: string = typeof value;

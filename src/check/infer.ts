...
export type RelationalValue =
  ...
  | {
    readonly tag: "record";
    readonly fields: ReadonlyMap<string, RelationalValue | null>;
  }
  | {
...
export interface TypeEnv {
  ...
  >;
  /** Checked runtime closures whose only published fact is a relation transform. */
  readonly relationshipFunctions: Map<
    string,
    { value: Value; typing: Typing }
  >;
  /**
...
export function lookupRelationshipFunction(
  env: TypeEnv,
  name: string,
): Value | null {
  const typing = lookupType(env, name);
  ...
  let scope: TypeEnv | null = env;
  while (scope !== null) {
    const found = scope.relationshipFunctions.get(name);
    if (found !== undefined) {
      if (found.typing !== typing) return null;
      return found.value;
    }
...
function recordRelationshipFunction(
  scope: TypeEnv,
  name: string,
  value: Value,
): void {
  if (relationshipSummary(value) === null) return;
  const typing = scope.names.get(name);
  if (typing === undefined) return;
  scope.relationshipFunctions.set(name, { value, typing });
}
...
function relationReferences(
  ...
  const values = relation.tag === "record" ? relation.fields : relation.cases;
...
function aliasedBinding(expr: Expr, scope: TypeEnv): number | null {
  ...
  if (
    head.args.length === 1 && head.callee.tag === "intrinsic" &&
  ...
  const callee = relationshipFunctionAt(head.callee, scope);
  if (callee === null) return null;
  const summary = relationshipSummary(callee);
  if (summary === null || summary.arity !== head.args.length) return null;
  return aliasFromTransform(summary.result, head.args, scope);
}
...
export function projectedBinding(
  ...
  return projectedIdentity(parent, expression.name, scope);
}
...
function recordProjectionAlias(
  scope: TypeEnv,
  parent: number,
  field: string,
  identity: number,
): void {
  scope.projections.set(`${parent}.${field}`, identity);
}

function relationshipFunctionAt(expr: Expr, scope: TypeEnv): Value | null {
  const path: string[] = [];
  let current = expr;
  ...
  let value = lookupComptime(scope, current.name);
  if (value === null) value = lookupRelationshipFunction(scope, current.name);
...
function aliasFromTransform(
  transform: RelationshipTransform,
  arguments_: readonly Expr[],
  scope: TypeEnv,
): number | null {
  if (transform.tag === "parameter") {
    const argument = arguments_[transform.parameter];
    return argument === undefined ? null : aliasedBinding(argument, scope);
  }
  if (transform.tag === "project") {
    const parent = aliasFromTransform(transform.target, arguments_, scope);
    if (parent === null) return null;
    return projectedIdentity(parent, transform.field, scope);
  }
  if (transform.tag === "payload") {
    const parent = aliasFromTransform(transform.target, arguments_, scope);
    if (parent === null) return null;
    return projectedIdentity(parent, `#${transform.constructor}`, scope);
  }
  return null;
}

function recordTransformProjectionAliases(
  transform: RelationshipTransform,
  root: number,
  arguments_: readonly Expr[],
  scope: TypeEnv,
): void {
  if (transform.tag !== "tuple" && transform.tag !== "record") return;
  const entries: readonly [string, RelationshipTransform | null][] =
    transform.tag === "tuple"
      ? transform.elements.map((element, index) => [String(index), element])
      : [...transform.fields];
  for (const [field, value] of entries) {
    if (value === null) continue;
    const alias = aliasFromTransform(value, arguments_, scope);
    if (alias !== null) {
      recordProjectionAlias(scope, root, field, alias);
      continue;
    }
    const projected = projectedIdentity(root, field, scope);
    recordTransformProjectionAliases(
      value,
      projected,
      arguments_,
      scope,
...
function recordExpressionProjectionAliases(
  expression: Expr,
  root: number,
  scope: TypeEnv,
): void {
  if (expression.tag === "tuple") {
    for (const [index, element] of expression.elements.entries()) {
      const alias = aliasedBinding(element, scope);
      if (alias !== null) {
        recordProjectionAlias(scope, root, String(index), alias);
        continue;
      }
      recordExpressionProjectionAliases(
        element,
        projectedIdentity(root, String(index), scope),
        scope,
      );
    }
    return;
  ...
  if (expression.tag === "shape") {
    for (const member of expression.members) {
      if (member.tag !== "field") continue;
      const alias = aliasedBinding(member.value, scope);
      if (alias !== null) {
        recordProjectionAlias(scope, root, member.name, alias);
        continue;
      }
      recordExpressionProjectionAliases(
        member.value,
        projectedIdentity(root, member.name, scope),
        scope,
      );
    }
    return;
  }
  const application = spine(expression);
  if (application === null) return;
  const callee = relationshipFunctionAt(application.callee, scope);
...
function recordPatternProjectionIdentities(
  pattern: Pattern,
  parent: number,
  scope: TypeEnv,
  context: Context,
  ...
  if (pattern.tag === "name") {
    const identity = recordBinding(scope, pattern.name, parent);
    context.patternBindings.set(pattern, identity);
    return;
  }
  if (pattern.tag === "tuple" || pattern.tag === "array") {
    for (const [index, element] of pattern.elements.entries()) {
      recordPatternProjectionIdentities(
        element,
        projectedIdentity(parent, String(index), scope),
        scope,
        context,
      );
    }
  ...
  if (pattern.tag === "shape") {
    for (const field of pattern.fields) {
      recordPatternProjectionIdentities(
        field.pattern,
        projectedIdentity(parent, field.name, scope),
        scope,
        context,
      );
    }
  ...
  if (pattern.tag === "constructor" && pattern.payload !== null) {
    recordPatternProjectionIdentities(
      pattern.payload,
      projectedIdentity(parent, `#${pattern.name}`, scope),
      scope,
      context,
    );
  }
  ...
  switch (expr.tag) {
    ...
    case "field": {
      ...
      if (member !== null && !member.ordinary) {
        ...
        // The member exists, but its result is available only once a concrete
        // specialization can run it. Keep that availability fact outside the
        // lattice: semantic `top` means every value is admitted, which is not
        // what a computation the checker cannot run has proved.
        return freshVar(level, "staged-computation");
      }
...
type Requirement = CanonicalRequirement | PredicateRequirement;

/**
 * Evaluates source syntax into the only two requirement forms the kernel has.
 *
 * `sig` and `@satisfies` deliberately share this boundary. A signature accepts
 * only the canonical form because it must support bidirectional checking;
 * `@satisfies` additionally accepts an observational predicate over a closed
 * subject. No source closure is ever inserted into the constraint graph.
 */
function resolveRequirement(
  expression: Expr,
  context: Context,
  use: "signature" | "satisfies",
): Requirement | null {
  const value = comptime(expression, context);
  if (value === null) {
    // A requirement inside a compile-time closure may name a loop variable
    // that specialization has not supplied yet. Its concrete call revisits the
    // expression; a runtime obligation may never defer this way.
    if (use === "satisfies" && context.phase === "comptime") return null;
    fail(
      "BLOT_SIG_NOT_COMPTIME",
      use === "signature"
        ? "A `sig` requirement must evaluate at compile time."
  ...
  const expected = bridge(value);
  if (expected !== null) return { tag: "type", value, type: expected };
  if (use === "signature") {
    fail(
      "BLOT_SIG_NOT_A_TYPE",
      `A \`sig\` requirement must evaluate to a type; this one evaluates to ${
        show(value)
      }.`,
      expression.span,
    );
  }
  return { tag: "predicate", value };
}

/** The single semantic judgment `subject satisfies requirement`. */
function applyRequirement(
  subjectType: SimpleType,
  ...
  if (requirement.tag === "type") {
    located(obligationSpan, () => constrain(subjectType, requirement.type));
    return subjectType;
  }

  const subject = reify(subjectType);
  if (subject === null) {
    fail(
      "BLOT_TYPE_NOT_REIFIABLE",
      `\`${
        showType(subjectType)
  ...
  if (
    ...
    fail(
      "BLOT_TYPE_ERROR",
      `A requirement must be a type value or a compile-time predicate from a closed type to \`Bool\`; this is ${
        show(requirement.value)
      }.`,
      requirementSpan,
    );
  }
  if (answer.name === "False") {
    fail(
      ...
      `\`${showType(subjectType)}\` does not satisfy the predicate.`,
      obligationSpan,
    );
  }
  return subjectType;
 ...
 *
 * When an argument is only known at run time there is no value to read. That is
 * an availability judgment, not semantic `⊤`: an evidenced inference variable
 * can keep flowing until specialization, but cannot authorize residual runtime
 * work or prove an arbitrary signature.
 */
function inferMemberApplication(
  ...
  if (found.ordinary) {
    ...
    needsSpecialization = containsStagedType(target) ||
      structuralAlternatives > 1 ||
...
function inferDeclarations(
  ...
  for (const declaration of declarations) {
    ...
    if (declaration.kind === "sig") {
      ...
      expect(
        requirement !== null && requirement.tag === "type",
        "a signature resolver returned a non-canonical requirement",
      );
      const unrepresentable = unrepresentableInteger(requirement.type);
      ...
      pendingSig = {
        ...
        requirement,
        requirementSpan: declaration.value.span,
    ...
    if (
      ...
      signature = pendingSig.requirement.type;
    }
    ...
    if (
      ...
      const expected = pendingSig.requirement.type;
      if (
        containsStagedType(type as Typing) ||
        expressionUsesStagedType(declaration.value, context)
      ) {
        fail(
          "BLOT_REFLECTION_NOT_INDEXED",
          `The inferred value for \`${pendingSig.name}\` depends on a result available only after compile-time specialization. Such a result can prove a signature only after its inputs are known at compile time.`,
          declaration.span,
        );
      }
      applyRequirement(
        instantiate(type as Typing, level, context.staging.instances),
        pendingSig.requirement,
        context,
        declaration.value.span,
        pendingSig.requirementSpan,
        declaration.span,
      );
      type = expected;
      pendingSig = null;
    ...
    if (declaration.pattern.tag === "name") {
      const root = lookupBinding(context.types, declaration.pattern.name);
      if (root !== null) {
        recordExpressionProjectionAliases(
          declaration.value,
          root,
    ...
    if (
      ...
      recordRelationshipFunction(
        context.types,
        declaration.pattern.name,
        {
          tag: "closure",
          parameter: declaration.value.parameter,
          body: declaration.value.body,
          env: context.values,
          self: null,
          source: declaration.value,
          deferred: declaration.value.deferred,
        },
      );
    }
...
function expressionUsesStagedType(
  expression: Expr,
  context: Context,
): boolean {
  for (const [candidate, type] of context.expressionTypes) {
    ...
    if (containsStagedType(type)) return true;
  }
  return false;
}
...
function containsStagedType(
  ...
  if (type.tag === "var") {
    ...
    return [...type.lower, ...type.upper].some((bound) =>
      containsStagedType(bound, seen)
    );
  }
  if (type.tag === "forall") {
    return containsStagedType(type.body, seen);
  }
  if (type.tag === "fun") {
    return containsStagedType(type.param, seen) ||
      containsStagedType(type.effects, seen) ||
      containsStagedType(type.result, seen);
  }
  if (type.tag === "record") {
    return [...type.fields.values()].some((field) =>
      containsStagedType(field, seen)
    );
  }
  if (type.tag === "array") {
    return containsStagedType(type.element, seen);
  }
  if (type.tag === "variant") {
    return [...type.cases.values()].some((payload) =>
      containsStagedType(payload, seen)
    );
  }
  if (type.tag === "union") {
    return type.members.some((member) => containsStagedType(member, seen));
  }
  return false;
}
...
function requireEvidencedRuntimeType(
  ...
  if (context.phase !== "runtime" || !containsStagedType(type)) return;
  if (comptime(expression, context, foldedEnv(context)) !== null) return;
  fail(
    "BLOT_REFLECTION_NOT_INDEXED",
    "A result available only after compile-time specialization cannot determine a runtime operation. Make its inputs known at compile time so the result has exact type evidence.",
    expression.span,
  );
}
...
function snapshotTypeEnv(
  ...
  for (const [name, entry] of source.relationshipFunctions) {
    if (omittedNames.has(name)) continue;
    snapshot.relationshipFunctions.set(name, {
      value: entry.value,
      typing: snapshotTyping(entry.typing, types, typings),
    });
  }

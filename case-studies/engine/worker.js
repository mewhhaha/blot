...
//
// Blot Core Wasm ABI 4 calls host effects synchronously, while drawing is paced
...
function writeText(scope, resultPointer, value) {
  const bytes = new TextEncoder().encode(value);
  const pointer = Number(
    instance.exports.cabi_realloc(scope, 0, 0, 1, bytes.length),
  );
...
const imports = {
  "blot:host/Canvas": {
    ...
    sprite(_scope, depth, size, texturePointer, textureLength, x, y) {
      projectedBatch.push({
        kind: "sprite",
        depth: Number(depth),
  ...
  "blot:host/Assets": {
    ...
    entry(scope, id, resultPointer) {
      ...
      // field order.
      writeText(scope, resultPointer + 48, entity.texture);
      const view = new DataView(instance.exports.memory.buffer);
  ...
  "blot:host/Host": {
    option(_scope, key, namePointer, nameLength) {
      self.postMessage({
        kind: "option",
        key: Number(key),
    ...
    seed: () => 424242n,
    selected(_scope, key, namePointer, nameLength) {
      self.postMessage({
        kind: "selected",
        key: Number(key),
        name: readText(namePointer, nameLength),
...
self.onmessage = async (event) => {
  ...
  try {
    const frames = instance.exports[message.export](scope);

...
const outputs = [
  ["experiments/rust-middle/src/schema.rs", source],
  ["experiments/rust-middle/src/frontend_plan.rs", frontendPlan],
] as const;
if (Deno.args.includes("--check")) {
  for (const [outputPath, expected] of outputs) {
    ...
    if (current !== expected) {
      throw new Error(
        `${outputPath} is stale; run \`deno task generate:rust-middle-schema\``,
      );
    }
  }
} else {

...
let is_some = fn option => case option of
  ...
  _ => False
sig at = Int -> Bool
let at = fn value => if value > 0 then is_some (Some value)
else is_some None
return { .at = at; }
`,
  );
...
ready <- Source.ready ()
return if unlikely ready then 1
else 2
`,
  );
  const built = await build(path);
...
Deno.test("a `let` recursive group reaches WebAssembly", async () => {
  ...
  await Deno.writeTextFile(
    path,
    `open @import "blot:prelude" ()
let is_even = rec (fn n => if n == 0 then 1
else is_odd (n - 1))
let is_odd = rec (fn n => if n == 0 then 0
else is_even (n - 1))
return is_even 11
`,
  );
...
Deno.test("a recursive group's members keep what they captured", async () => {
  ...
  await Deno.writeTextFile(
    ...
    `open @import "blot:prelude" ()
let total = fn step =>
  let down = rec (fn n => if n < step then 0
  else n + up (n - step))
...
Deno.test("a `const` recursive group reaches WebAssembly", async () => {
  ...
  await Deno.writeTextFile(
    path,
    `open @import "blot:prelude" ()
const even = rec (fn n => if n == 0 then 1
else odd (n - 1))
const odd = rec (fn n => if n == 0 then 0
else even (n - 1))

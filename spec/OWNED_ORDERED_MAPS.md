...
`OrderedTextMap` is a finite map from `Text` to one value type. Its persistent
representation is an array of entries:
...
uniqueness. The owned representation is a `Slice.of (Text, V)`, but its public
operations expose keys and values rather than positional mutation.
...
partition, exact witnesses, element replacement, and proof erasure. A sorted
map refines those intervals into key ranges:
...
The invariant is established before `Slice.claim`. Failed acquisition returns
the original array and mints no authority.

## 3. Public authority and operations

`OrderedTextMap.of V` is an opaque linear authority represented by
`Slice.of (Text, V)`.

```text
claim
  : [(Text,V)]
  -> #Claimed (OrderedTextMap.of V)
   | #NotStrictlyOrdered [(Text,V)]
...
replace
  ...
  -> #Replaced (V, !OrderedTextMap.of V)
   | #Missing (V, !OrderedTextMap.of V)
...
`get` borrows the authority. As with `Slice.get`, the ownership checker
rejects a borrowed read that would copy an owned value. The first adapter
therefore admits only values carrying no affine or linear obligation:
acquisition validation and binary search inspect an entry pair in order to read
its key, and the current source language has no field-borrow operation that can
project the key without also producing the value.
...
contains keys strictly less than `key`; the right contains keys greater than
or equal to it. The lower bound is always in `[0,length]`, so the underlying
interval split cannot fail in a checked implementation.

`join` consumes the exact witness and exact siblings minted by the split.
Reassociation delegates to the same witness operations as `Slice`.
`freeze` is defined only for a complete root, as it delegates to
`Slice.freeze`.
...
Let `I(S, lo, hi)` be the existing array-interval authority and let
`ordered(S)` be the full-Store invariant. Define:

```text
MapRange(S, lo, hi) = I(S, lo, hi) + ordered(S)
```

The `ordered(S)` fact is persistent compiler knowledge established at
acquisition. It authorizes no mutation on its own.
...
trusted source-level refinement. It does not claim to be the future
...
physical order. It would require family-tagged serialized ownership values and
a runtime representation for non-contiguous membership. This adapter chooses
...
1. accepted and rejected acquisition;

...
and subsequent encoder writes. UTF-8 decoding remains strict, length checks
remain bounded by the supplied view, and trailing bytes remain an error. A
response may be decoded directly from the current guest-memory view only while
no guest call can invalidate it; any payload escaping that synchronous decode
must be copied before the next transport call. These allocation changes do not
change frame schema, compiler-host ABI, or semantic cache identity.
...
The checked-in prelude snapshot must be identical to the snapshot regenerated by
the pinned compiler build. CI checks its tracked diff immediately after that
build; it must not hide stale tracked bytes by packaging only the regenerated
copy. The regenerated snapshot digest must still match the compiler artifact
manifest before host installation.

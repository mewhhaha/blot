...
| width subtyping          | runtime lambdas specialize projections, destructuring, nested record/tuple/array leaves, aliases, imports, static higher-order selection, and run-time choices per concrete call | specialization closes representation for every call                    | represent structural functions produced only by unknown run-time higher-order control          |
...
result, rather than forcing one nominal parameter representation. Functions
produced only by unknown run-time higher-order control still need a closed

...
impl ResidualTrace {
    ...
    pub(crate) fn primitive(
        ...
        let value = match name {
            ...
            "@region.array.claim" if arguments.len() == 1 => {
                ...
                let zero = self.constant(WireConstant::SignedInteger64("0".to_owned()), 4, span);
            ...
            "@region.array.get" if arguments.len() == 2 => {
                ...
                let absolute =
                    self.operation("scalar", 4, vec![start.id, index.id], span, Some("add"));
            ...
            "@region.array.swap" if arguments.len() == 3 => {
                ...
                let left = self.operation("scalar", 4, vec![start.id, left.id], span, Some("add"));
                let right =
                    self.operation("scalar", 4, vec![start.id, right.id], span, Some("add"));
                let RuntimeType::Store { element_type } = self.types[store.type_id] else {
                    return Err(hir_error("Region Store projection lost its Store type."));
                };
    ...
    fn split_region(
        ...
        let length = self.operation("scalar", 4, vec![end.id, start.id], span, Some("subtract"));
        ...
        let middle = self.operation("scalar", 4, vec![start.id, offset.id], span, Some("add"));

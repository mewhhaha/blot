...
pub unsafe extern "C" fn lower_source(source_pointer: *const i32, source_unit_count: u32) -> u32 {
    ...
    let lowered =
        decode_source_words(source_words).and_then(|source| match frontend::ingest(&source) {
            Ok(program) => lower_units(&source, &program.tokens, &program.nodes, &program.edges)
                .map(|module| serde_json::json!({ "ok": true, "module": module })),
            Err(diagnostics) => Ok(serde_json::json!({
                "ok": false,
...
fn lower_units(
    ...
    let cst = CompactCst::new(source, tokens, nodes, edges, RULE_NAMES)?;
    lower::lower_module(&cst)
}

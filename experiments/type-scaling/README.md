...
Each row also includes resident Rust checker work schema 1: unique type nodes,
recursive intern attempts, constraints, settle/freshen/union visits, boundary
materializations, closure free-name candidates, and captures actually bridged.
The executable scaling gate uses the semantic decisions whose required count is

...
Wasm units while keeping the page running. Run `pnpm example:spark-browser` for
an [event actor with explicit I/O](case-studies/spark-browser/README.md),
reusable workers, cancellation, and cleanup during reload. The source libraries
`blot:spark`, `blot:channel`, `blot:io`, `blot:events`, and `blot:shared`
provide structured concurrency and selected host services; the
